<template>
        ...
        <div class="form-search">
            <el-form @submit.prevent.stop :inline="true" size="mini">
                    ...
                    <el-input type="text" style="width:80px;" v-model="search.chongzhijine_start"></el-input>
-<el-input type="text" style="width:80px;" v-model="search.chongzhijine_end"></el-input>
                    
                </el-form-item>                <el-form-item label="充值人">
                
                    <el-input v-model="search.chongzhiren"></el-input>
        ...
        <el-table border :data="list" style="width: 100%" highlight-current-row>
            <el-table-column type="index" label="#"></el-table-column> <!-- 序号 -->
                ...
                <template slot-scope="{row}">
                    {{  row.chongzhijine  }}                </template>
            </el-table-column>
                        <el-table-column label="充值人"width="80">
                <template slot-scope="{row}">
                    {{  row.chongzhiren  }}                </template>
            </el-table-column>
            ...
            <el-table-column label="操作">
                <template slot-scope="{row}">
                    ...
                    <el-button icon="el-icon-edit" @click="$goto({path:'/admin/chongzhiupdt',query:{id:row.id } })"
    ...
    export default {
        ...
        methods: {
            ...
            loadList( page ){
                ...
                this.$post(api.chongzhi.listchongzhiren , filter).then(res=>{
                    this.loading = false;
            ...
            deleteItem( row ){
                this.$confirm('确定删除数据？' , '提示',{ // 弹出 确认框提示是否要删除
                    ...
                    this.$post(api.chongzhi.delete , {// 提交后台
                        id:row.id
                    }).then(res=>{
                        this.loading = false;
                        if(res.code != api.code.OK){
