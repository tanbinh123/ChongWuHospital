<template>
        ...
        <div class="form-search">
            <el-form @submit.prevent.stop :inline="true" size="mini">
                    ...
                    <el-select v-model="search.dingdanzhuangtai" ><el-option label="请选择" value=""></el-option><el-option label="待支付" value="待支付"></el-option>
<el-option label="支付成功" value="支付成功"></el-option>
        ...
        <el-table border :data="list" style="width: 100%" highlight-current-row>
            <el-table-column type="index" label="#"></el-table-column> <!-- 序号 -->
                ...
                <template slot-scope="{row}">
                    {{  row.shouhuodizhi  }}                </template>
            </el-table-column>
                        <el-table-column label="订单状态">
                <template slot-scope="{row}">
                    {{  row.dingdanzhuangtai  }}                </template>
            </el-table-column>
                ...
                <template slot-scope="{row}">
                    ...
                    <template slot-scope="{row}">
                        {{  row.iszf  }}
                                   ...
                                   @click="$goto({path:'/admin/pay' , query:{biao:'dingdanxinxi' , id:row.id,ordersn:row.dingdanhao,zongji:row.dingdanjine } })"
                                   v-if="row.iszf=='否'">
                            去支付
                        </el-button>
                                            </template>
            ...
            <el-table-column label="操作">
                <template slot-scope="{row}">
                    <el-button-group>
                        ...
                        <el-button @click="$goto({path:'/admin/dingdanxinxidetail',query:{id:row.id } })" icon="el-icon-info" type="info" size="mini"></el-button>
                        </el-tooltip>
                                            <el-tooltip content="编辑" placement="top">
                    <el-button icon="el-icon-edit" @click="$goto({path:'/admin/dingdanxinxiupdt',query:{id:row.id } })"
